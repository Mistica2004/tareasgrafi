...
while True:
    ...
    if result.multi_hand_landmarks:
        ...
        # Tamaño de vista según zoom
        zoom_w = int(VIEW_W * zoom)
        zoom_h = int(VIEW_H * zoom)

        zoom_w = max(300, min(zoom_w, SCENE_W))
        zoom_h = max(300, min(zoom_h, SCENE_H))

        # Movimiento de cámara
        offset_x = int(index_x * (SCENE_W - zoom_w))
        offset_y = int(index_y * (SCENE_H - zoom_h))

        offset_x = max(0, min(offset_x, SCENE_W - zoom_w))
        offset_y = max(0, min(offset_y, SCENE_H - zoom_h))
    ...
    # ==========================
    vista = scene[
        offset_y:offset_y + zoom_h,
        offset_x:offset_x + zoom_w
    ]
