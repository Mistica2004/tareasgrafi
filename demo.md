Proyecto: Demo Procedural 

Objetivo

Desarrollar una demostración gráfica procedural utilizando OpenCV y Python, aplicando conceptos matemáticos, transformaciones geométricas, curvas paramétricas, efectos visuales y generación de animaciones en tiempo real.

Descripción

En este proyecto se implementó una secuencia animada de diferentes escenas generadas matemáticamente. Cada escena utiliza ecuaciones y funciones paramétricas para crear formas geométricas, patrones visuales y efectos dinámicos sin utilizar modelos externos o imágenes prediseñadas.

La animación tiene una duración total de 60 segundos y presenta una transición gradual entre distintas escenas, generando una experiencia visual continua. Además, se incorporan efectos de postprocesamiento para mejorar la apariencia gráfica del resultado final.
...
* Curvas de Lissajous.
* Rosa polar.
* Espirógrafo (Hipotrocoide).
* Lemniscata.
* Espiral matemática.
* Cardioide.
* Transformaciones geométricas.
* Sistema de partículas.
* Simulación procedural de fuego.
* Transiciones suaves entre escenas.
* Efectos de viñeta.
* Efectos de scanlines.
* Posterización de imagen.
* Exportación automática a video MP4.
...
El programa divide la animación en múltiples escenas independientes. Cada escena utiliza ecuaciones matemáticas para generar figuras dinámicas que evolucionan con el tiempo.

Las curvas son calculadas mediante funciones paramétricas y posteriormente dibujadas utilizando las herramientas gráficas de OpenCV. Durante la ejecución se generan transiciones suaves entre escenas mediante interpolación y mezcla de imágenes.

Al finalizar la secuencia se presenta una simulación de fuego procedural acompañada de un mensaje de cierre.

Efectos visuales implementados

Degradados dinámicos

Se generan fondos mediante gradientes en espacio de color HSV, permitiendo cambios de ambiente y color durante la animación.

Viñeta

Se oscurecen progresivamente los bordes de la imagen para dirigir la atención hacia la zona central.

Scanlines

Se agregan líneas horizontales simulando una apariencia retro similar a pantallas CRT.

Posterización

Se reduce la cantidad de niveles de color para producir un estilo visual artístico.

Resultados

Se obtuvo una demostración visual completamente procedural compuesta por diversas escenas matemáticas animadas. El proyecto integra conceptos de programación gráfica, animación, procesamiento digital de imágenes y generación algorítmica de contenido visual.

Además, el sistema genera automáticamente un archivo de video en formato MP4 con la animación completa.

Conclusiones

Este proyecto permitió aplicar conocimientos de programación, matemáticas y gráficos por computadora para crear contenido visual dinámico sin depender de recursos externos. Se comprendió el uso de curvas paramétricas, transformaciones geométricas, sistemas de partículas y técnicas de postprocesamiento de imágenes utilizando OpenCV.

La implementación demuestra cómo las matemáticas pueden utilizarse para generar animaciones complejas y efectos visuales atractivos mediante programación procedural.
...
def scene_lemniscata(img, t):

    background_hsv_gradient(img, t, 40, 120)

    fx = lambda x: np.cos(x)/(1+np.sin(x)**2)
    fy = lambda x: np.sin(x)*np.cos(x)/(1+np.sin(x)**2)
...
def scene_spiral(img, t):
    ...
    background_hsv_gradient(img, t, 80, 160)
...
def scene_transform(img, t):

    background_hsv_gradient(
        img,
        t,
        0,
        80
    ...
    figura = cv2.transform(
        estrella.reshape((-1,1,2)),
        M
    )
...
def scene_fire(img, t, state):
    ...
    # Disminuir calor anterior
    heat[:] = heat * 0.93

    # Generar calor en la parte inferior
    base_n = 1400
    ...
    # Conversión a color
    h = (
        20 - 20 * np.clip(heat, 0, 1)
    ).astype(np.uint8)

    s = (
        220 - 80 * np.clip(heat, 0, 1)
    ).astype(np.uint8)

    v = (
        60 + 195 * np.clip(heat, 0, 1)
    ).astype(np.uint8)
...
def render_scene(buf, scene_id, t, rng, fire_state):
    ...
    if scene_id == 0:
        ...
        scene_lemniscata(buf, t)

    elif scene_id == 5:
        scene_spiral(buf, t)

    elif scene_id == 6:
        scene_cardioide(buf, t)

    elif scene_id == 7:
        scene_transform(buf, t)
...
def timeline(t, rng, bufA, bufB, fire_state):
    ...
    if block < NUM_SCENES-1 and t_in >= dur_scene-trans:
